...
comarca = ''
while not correct:
    comarca = input("Please choose a comarca(ex:Barcelonès): ")
    for i in range(len(data)):
        if comarca not in data[i]['comarca']:
            print('comarca {} is not correct, please try again'.format(comarca))
        else:
            correct = True
        break
...
for i in range(len(data)):
    if 'categoria' in data[i]:
        if data[i]['categoria'] == type and data[i]['comarca'] == comarca:
...
map.save('mapTask5.html')
webbrowser.open('mapTask5.html')
